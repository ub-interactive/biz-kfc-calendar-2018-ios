...
//    NSLog(@"setData  == %@", data);
    ...
    for (KFCPasterModel *pasterModel in data) {
...
//        btn.layer.masksToBounds = YES;        // 这句不写, 可使view的圆角与阴影共存
        ...
        KFCPasterModel *pasterModel = self.data[i];
...
//    }else{
        KFCPasterModel *pasterModel = self.data[self.preBtn.tag];
...
-(void)tagsButtonClicked:(UIButton *)btn{
    ...
    KFCPasterModel *pasterModel = self.data[btn.tag];
...
-(void)tagsButtontouchDown:(UIButton *)sender{
    ...
    KFCPasterModel *pasterModel = self.data[sender.tag];
